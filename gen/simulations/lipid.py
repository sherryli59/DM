...
class SingleLipid():
    def __init__(self, base_filename="test", temperature=323.15,
        ...
        ff = ForceField("lipid/charmm_files/custom_xml/charmm36_custom_all.xml")
        ...
        reporter = HDF5Reporter(self.folder_name + "_sim.h5", 1000, atomSubset=self.target_atom_indices)
    ...
    def generate_long_trajectory(self, num_data_points=int(1E8), save_freq=1000, tag = ""):
        ...
        for _ in range(num_data_points):
            self.run_sim(save_freq)
            positions, velocities, forces, pe, ke, pe_beta = self.get_information()
            f=open(tag + "_positions.txt",'ab')
            np.savetxt(f, positions)
            f.close()

            f=open(tag + "_velocities.txt",'ab')
            np.savetxt(f, velocities)
            f.close()

            f=open(tag + "_forces.txt",'ab')
            np.savetxt(f, forces)
            f.close()

            f=open(tag + "_pe.txt",'ab')
            np.savetxt(f, np.expand_dims(pe, 0))
            f.close()

            f=open(tag + "_pe_beta.txt",'ab')
            np.savetxt(f, np.expand_dims(pe_beta, 0))
            f.close()
            
            f=open(tag + "_ke.txt",'ab')
            np.savetxt(f, np.expand_dims(ke, 0))
            f.close()
